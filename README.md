...
七款修仙题材在线小游戏的合集门户。纯前端、免登录，进度保存在浏览器本地，支持 PWA 安装与离线游玩。七款游戏均使用本地托管的 Three.js：转转刀呈现实时演武场，其余六款以三维场景展示角色、阵容、战斗与路线，并连接原有玩法操作。
...
| 斩仙塔 | `games/cardtower.html` | 肉鸽卡牌爬塔：分支路线、牌组构筑、奇遇与营地 |
...
搜索与分类保存在地址栏，刷新、返回或分享链接均可恢复；已有离线缓存时，带筛选的首页链接也能离线打开。

设置中的存档导入会先展示文件名、备份时间以及新增、替换、相同的数据项数量，确认后再提交并刷新。取消预览不会写入数据；也可先导出当前存档留作备份。导出会采集修仙之路尚未自动保存的最新进度，切到后台、离开页面和更新前也会先保存；保存失败会阻止更新，导入成功后旧页面不会再把原角色写回。
...
场景支持鼠标或触屏点选、拖动转向、缩放与视角复位。聚焦画布后，左右方向键选择目标，回车确认；卡牌说明、角色属性和原有操作面板继续提供完整信息。场景直接读取游戏状态，血量、手牌、阵位、迷雾和路线随真实进度更新。切到后台或画布滚出视口时暂停绘制，离开页面时释放图形资源；山川、庭院等布景只在玩法或主题切换时重建，地块、路线与路标随进度更新。WebGL 初始化或运行错误会明确显示。

- **修仙之路**：云台庭院显示修炼进度，点选修士开始或停止修炼；历练时点选妖兽攻击，战斗结束后点选修士返回。下一境界、突破条件和宗门任务奖励在面板实时更新。
- **仙途模拟器**：浮岛修士随年龄阶段变化，点选事件路标作出选择或继续人生；选择后保存并显示真实属性变化。手札可以按年龄阶段和关键词检索，结算后仍可翻阅。
- **鬼谷八荒**：三维地形显示身边已探索区域，点选地块预览路线，点选修士沿路线前进一格；月度计划、坐骑、行旅耗时和遭遇继续生效。原地图保留附近、全图及八向操作。
- **斩仙塔**：三幕分支路线显示真实相连的遭遇、精英、奇遇、商店、营地和守关节点，每层选择一路。点选发光节点进入，战斗中点选手牌出招，敌方意图同步显示。数字键出牌或选择奖励，E 结束回合；手机横滑查看卡牌详情。
- **灵卡对决**：三维棋盘可点选手牌召唤弟子或选择法术，再点选合法目标攻击或施法；费用、嘲讽和回合限制沿用原规则。数字键出牌、E 结束回合、H 释放灵技，Escape 取消目标选择；组牌面板显示费用曲线。
- **仙卡录**：点选三维阵位换将；秘境开战后场景进入战斗弹层，可点选敌人锁定目标、点选大招就绪的仙友释放技能。31 位角色保留本地矢量立绘和排序、一键上阵、换将比较；秘境共 30 章。
...
js/game-scene-*.js    六款游戏的只读视图模型、注册桥接和原操作适配（UMD）
js/game-three-*.js    共享的 Three.js 场景、拾取、相机、角色与资源生命周期
js/canvas-shapes.js   标签绘制共用的 Canvas 2D 圆角矩形（含旧版 Safari 回退）
js/vendor/            固定版本的本地 Three.js 及其许可证
js/<game>.js          各游戏逻辑
js/*-recovery.js      从游戏中抽出的纯逻辑模块（UMD 包装，可在 Node 中单测）
js/*-progression.js
css/shared.css        共享样式；css/<game>.css 各游戏样式
css/game-three.css    六款游戏的三维场景布局与移动端样式
...
在联网状态下打开一次首页，等待 Service Worker 安装完成，即会预缓存所有游戏及其本地依赖。随后可以离线打开这些入口，字体加载不影响玩法。带版本号的脚本、样式和图形资源命中预缓存后直接返回，不再逐个回源核对；页面本身仍优先取网络，离线时回退到最近一次成功打开的版本。
